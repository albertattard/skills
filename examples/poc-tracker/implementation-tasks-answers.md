...
- `application-platform`: Java 25, Spring Boot, Maven project setup, server-rendered or Java-application-owned UI, H2 file persistence, and manager dropdown identity.
...
A: The first release should cover the manager triage workflow: managers can select their name from a dropdown, see current PoCs, identify blocked or stale work, and open the PoC context. Include PoC creation/editing, journal entries, and state transitions only where they are needed to keep the triage view accurate and verifiable. Do not implement real authentication for the MVP.
