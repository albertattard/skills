...
The application should persist its database file on a persistent volume mounted into the container at runtime.
...
Use Playwright for Java, not the Node.js Playwright package, for functional tests against the running containerised web application.

The functional test command should be exposed through Maven, for example as a functional-test profile that starts from a running container URL and can be executed by GitHub Actions. The generated project should define the exact Maven command, such as `./mvnw verify -Pfunctional-tests`, once the project structure exists.
