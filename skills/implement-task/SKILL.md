...
For browser-level tests that cover multi-step user flows, use a small domain-specific fluent driver or page object by default unless the flow has only one or two interactions or the repository already has a different convention. Do not leave raw browser automation objects such as Playwright `Page`, `Locator`, selectors, waits, and repeated action helpers in the test method when a scenario spans multiple user actions. The test should read like a product workflow, while the driver owns browser mechanics and product-state assertions. Name driver methods after user actions and observable product states, not DOM operations. Keep fluent driver assertions focused on named product states; avoid `assertPageIsCorrect`-style helpers that assert incidental page content. Keep the driver small at first, and split it by screen or workflow only when one class starts mixing unrelated responsibilities.
